...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        self.params["W1"] = W1.to(device, dtype=dtype)
        self.params["b1"] = b1.to(device, dtype=dtype)
        self.params["W2"] = W2.to(device, dtype=dtype)
        self.params["b2"] = b2.to(device, dtype=dtype)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out, l1_cache = Linear.forward(X, self.params["W1"], self.params["b1"])
        out, relu_cache = ReLU.forward(out)
        scores, l2_cache = Linear.forward(out, self.params["W2"], self.params["b2"])
        ...
        # Replace "pass" statement with your code
        N = scores.shape[0]
        scores1 = scores - scores.max(dim=1, keepdim=True).values
        e_scores = scores1.exp()
        p_scores = e_scores / e_scores.sum(dim=1, keepdim=True)
        loss = torch.mean(-1 * p_scores[range(N), y].log())
        loss += torch.sum(self.params["W1"] ** 2) * self.reg
        loss += torch.sum(self.params["W2"] ** 2) * self.reg

        ds = p_scores.clone()
        ds[range(N), y] -= 1  # [N,C]
        ds /= N

        dx, dw, db = Linear.backward(ds, l2_cache)
        grads["W2"] = dw + 2 * self.reg * self.params["W2"]
        grads["b2"] = db
        dx = ReLU.backward(dx, relu_cache)

        dx, dw, db = Linear.backward(dx, l1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for hidden_dim in hidden_dims:
            self.params[f"W{i}"] = torch.randn(input_dim, hidden_dim, device=device, dtype=dtype) * weight_scale
            self.params[f"b{i}"] = torch.zeros(hidden_dim, device=device, dtype=dtype)
            input_dim = hidden_dim
            i += 1
        self.params[f"W{i}"] = torch.randn(input_dim, num_classes, device=device, dtype=dtype) * weight_scale
        self.params[f"b{i}"] = torch.zeros(num_classes, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        for n in range(self.num_layers - 1):
            ...
            if self.use_dropout:
                out, dropout_cache[f"{i}"] = Dropout.forward(out, self.dropout_param)
            i += 1

        out, cache = Linear.forward(out, self.params[f"W{i}"], self.params[f"b{i}"])
        cache_list.append(cache)
        scores = out
        #################################################################
        #                      END OF YOUR CODE                         #
        ...
        for i in range(self.num_layers):
            loss += 0.5 * (self.params[f"W{i + 1}"] ** 2).sum() * self.reg

        dx, dw, db = Linear.backward(dx, cache_list[self.num_layers - 1])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dx = Dropout.backward(dx, dropout_cache[f"{i}"])
            dx, dw, db = Linear_ReLU.backward(dx, cache_list[i - 1])
            grads[f"W{i}"] = dw + self.params[f"W{i}"] * self.reg
            grads[f"b{i}"] = db
...
def rmsprop(w, dw, config=None):
    ...
    cache = decay_rate * cache + (1 - decay_rate) * dw.square()
    next_w = w - lr / (cache.sqrt() + epsilon) * dw
    config["cache"] = cache
...
def adam(w, dw, config=None):
    ...
    m = b1 * m + (1 - b1) * dw
    mt = m / (1 - b1 ** t)
    v = b2 * v + (1 - b2) * dw.square()
    vt = v / (1 - b2 ** t)
    next_w = w - lr * mt / (vt.sqrt() + epsilon)
    config["m"] = m
    config["v"] = v
    config["t"] = t
    config["beat1"] = b1
