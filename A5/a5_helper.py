...
def coord_trans(bbox, w_pixel, h_pixel, w_amap=7, h_amap=7, mode='a2p'):
    ...
    resized_bbox = bbox.clone()  # [B,X,4]
    # could still work if the first dim of bbox is not batch size [X,1,4]
    # in that case, w_pixel and h_pixel will be scalars
    resized_bbox = resized_bbox.view(bbox.shape[0], -1, bbox.shape[-1])  # [B, X, 4]
    invalid_bbox_mask = (resized_bbox == -1)  # indicating invalid bbox

    if mode == 'p2a':
        # pixel to activation
        width_ratio = w_pixel * 1. / w_amap  # [B]
        height_ratio = h_pixel * 1. / h_amap  # [B]
...
def GenerateGrid(batch_size, w_amap=7, h_amap=7, dtype=torch.float32, device='cuda'):
    ...
    """
    w_range = torch.arange(0, w_amap, dtype=dtype, device=device) + 0.5  # [7]
    h_range = torch.arange(0, h_amap, dtype=dtype, device=device) + 0.5  # [7]

    w_grid_idx = w_range.unsqueeze(0).repeat(h_amap, 1)  # [7,7]
    h_grid_idx = h_range.unsqueeze(1).repeat(1, w_amap)  # [7,7]
    grid = torch.stack([w_grid_idx, h_grid_idx], dim=-1)  # [7,7,2]
    grid = grid.unsqueeze(0).repeat(batch_size, 1, 1, 1)  # [1,7,7,2]->[B,7,7,2]
...
def ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.3, method='FasterRCNN'):
    ...
    if method == 'FasterRCNN':
        ...
        # anchor [B,A,H,W,4]
        # bboxes [B,N,5] GT 坐标点和类别标签
        # grid [B,H,W,2] 网格中心点
        # iou_mat [B,A*H*W,N]

        # 记录有真实标注的布尔索引
        bbox_mask = (bboxes[:, :, 0] != -1)  # BxN, indicate invalid boxes
        # 生成真实标注的中心点 -1的点变成(0,0) -> [B,N,2]
        bbox_centers = (bboxes[:, :, 2:4] - bboxes[:, :, :2]) / 2. + bboxes[:, :, :2]  # BxNx2

        # abs(每一个网格中心点[B,H*W,1,2] - 每一个GT的中心点[B,1,N,2]) -> (B,H*W,N,2) -> sum(),dim=-1 => (B,H*W,N)
    ...
    wh_offsets = torch.log(
        (bboxes[:, 2:4] - bboxes[:, :2]) / (activated_anc_coord[:, 2:4] - activated_anc_coord[:, :2])
    )
...
def DetectionInference(detector, data_loader, dataset, idx_to_class, thresh=0.8, nms_thresh=0.3, output_dir=None,
    ...
    with torch.no_grad():
        ...
        for iter_num, data_batch in enumerate(data_loader):
            ...
            final_proposals, final_conf_scores, final_class = detector.inference(images,
                                                                                 thresh=thresh,
            ...
            for idx in range(batch_size):
                ...
                if output_dir is not None:
                    ...
                    with open(os.path.join(det_dir, file_name), 'w') as f_det, \
                        ...
                        print(
                            '{}: {} GT bboxes and {} proposals'.format(img_ids[idx], valid_box,
                                                                       resized_proposals.shape[0]))
