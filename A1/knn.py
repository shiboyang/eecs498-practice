...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        x = torch.flatten(x_train[i])
        for j in range(num_test):
            y = torch.flatten(x_test[j])
            # squared euclidean distance
            dists[i, j] = torch.sqrt(torch.sum((x - y) ** 2))
            # manhattan distance
            # dists[i, j] = torch.sum((x - y).abs())
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        x = torch.flatten(x_train[i])
        y = torch.flatten(x_test, 1)
        _ret = torch.sqrt(torch.sum((x - y) ** 2, dim=1))
        dists[i] = _ret.view(-1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x1 = x_train.reshape(num_train, -1)
    x2 = x_test.reshape(num_test, -1)
    ret = torch.sum(x1 ** 2, dim=1).reshape(-1, 1) + torch.sum(x2 ** 2, dim=1) - torch.mm(x1, x2.t()) * 2
    dists = torch.sqrt(ret)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ret = torch.topk(dists[:, i], k, largest=False)
        ret = y_train[ret.indices]
        y_pred[i] = torch.bincount(ret).argmax()
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_list = [x_train_folds[_i] for _i in range(num_folds) if _i != i]
            x = torch.cat(x_list)
            y_list = [y_train_folds[_i] for _i in range(num_folds) if _i != i]
            y = torch.cat(y_list)
            ...
            p = knn.check_accuracy(x_test, y_test, k, quiet=True)
            accuracy.append(p)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    k_list = list(k_to_accuracies.keys())
    acc_list = list(k_to_accuracies.values())
    max_acc_index = torch.tensor(acc_list).mean(dim=1).argmax()
    best_k = k_list[max_acc_index]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
