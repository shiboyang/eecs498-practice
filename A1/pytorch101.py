...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for (i0, i1), v in zip(indices, values):
        x[i0, i1] = v
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    start = (start // 10 + 1) * 10
    stop = (stop // 10 + 1) * 10
    x = torch.arange(start, stop, step=10, dtype=torch.double)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    C = max(x) + 1
    R = len(x)
    y = torch.zeros((R, C), dtype=torch.float32)
    idx0 = torch.arange(R)
    idx1 = torch.tensor(x, dtype=torch.int64)
    y[idx0, idx1] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    minimum_idx = torch.argmin(y, dim=1)
    idx0 = torch.arange(x.shape[0])
    y[idx0, minimum_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
