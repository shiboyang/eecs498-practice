...
def mutate_tensor(x, indices, values):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    assert len(indices) == len(values), f'len(indices) == len(values)'
    for n, v in zip(values, indices):
        i, j = v
        x[i, j] = n
    #############################################################################
    #                            END OF YOUR CODE                               #
...
def slice_indexing_practice(x):
    ...
    # Replace "pass" statement with your code
    m, n = x.shape
    # last_row = torch.tensor([x[-1, :].tolist() for i in range(n)])
    last_row = x[-1, :]
    third_col = x[:, 2:3]
...
def shuffle_cols(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    y = torch.zeros((x.shape[0], 4), dtype=torch.int)
    y[:, :2] = x[:, [0, 0]]
    y[:, 2] = x[:, 2]
    y[:, 3] = x[:, 1]
    #############################################################################
...
def reverse_rows(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    ids = torch.arange(x.shape[0]).tolist()
    y = x[ids.sort(reverse=True), :]
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    return y
