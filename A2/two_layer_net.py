...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1).values.view(-1, 1)  # [N,C]
    e_score = scores.exp()
    p_score = e_score / e_score.sum(dim=1).view(-1, 1)  # [N,C]
    l1 = torch.mean(p_score[range(N), y].log() * -1)
    loss = l1 + reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2))
    ...
    ds2[range(N), y] -= 1
    ds2 /= N
    grads["W2"] = h1.t() @ ds2 + 2 * reg * W2
    grads["b2"] = ds2.sum(dim=0)

    ds1 = ds2 @ W2.t()
    ds1 = ds1 * (h1 > 0)
    grads["W1"] = X.t() @ ds1 + 2 * reg * W1
    grads["b1"] = ds1.sum(dim=0)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.5, 1.0, 0.1]
    hidden_sizes = [8, 64, 128, 512]
    regularization_strengths = [1e-5, 1e-3, 1, 1e-4]
        ...
        for lr in learning_rates:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    model = TwoLayerNet(3 * 32 * 32, hd, 10, device=x_train.device, dtype=x_train.dtype)
                    state = model.train(x_train, y_train, x_val, y_val, lr, lrd, reg)
                    ...
                    print(f"hd:{hd}, reg:{reg}, lr:{lr}, lrd:{lrd}, acc:{val_acc}")
