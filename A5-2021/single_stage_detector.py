...
def GenerateAnchor(anc, grid):
    ...
    A = anc.shape[0]
    shift_x = (anc[:, 0] / 2.0).reshape([1, A, 1, 1, 1])  # [1,A,1,1,1]
    shift_y = (anc[:, 1] / 2.0).reshape([1, A, 1, 1, 1])  # [1,A,1,1,1]
    shift_xy = torch.cat([shift_x, shift_y], dim=-1)
    anchors_c1 = grid.unsqueeze(1)
    anchors_c2 = grid.unsqueeze(1)
    anc_tl = anchors_c1 - shift_xy
    anc_br = anchors_c2 + shift_xy
    anchors = torch.cat([anc_tl, anc_br], dim=-1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def GenerateProposal(anchors, offsets, method='YOLO'):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    proposals = anchors[..., :2].clone()  # [B,A,H,W,2]
    anchors_wh = anchors[..., [2, 3]] - anchors[..., [0, 1]]  # [B,A,H,W,2]
    proposals += anchors_wh / 2.0  # [B,A,H,W,2]

    if method == "YOLO":
        proposals_xy = proposals + offsets[..., :2]
        proposals_wh = anchors_wh * offsets[..., 2:].exp()
    else:
        proposals_xy = proposals + offsets[..., :2] * anchors_wh
        proposals_wh = anchors_wh * offsets[..., 2:].exp()

    proposals = torch.cat([proposals_xy - proposals_wh / 2.0, proposals_xy + proposals_wh / 2.0], dim=-1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
